  ...
  bot.botCommands.register({
    ...
    async execute(rawData, { args }, lang) {
      ...
      switch (param) {
        ...
        case "reset": {
          const user = await bot.getUser(rawData.uid);

          //if (!isObject(user) || await bot.utils.getRole(user) <= ROLE.MANAGER) return false;

          await bot.redis.removeCommandFromCoolDown("main", "roulette@start", "perUse");
        ...
        case "start": {
          ...
          //if (isWeekend) {
          if (isWeekend  && !isDecember) {
            bot.chat(bot.utils.replace(lang.roulette.startingWeekend, {})); //.delay(duration * 1e3).call("delete");
          }
          ...
          bot.chat(bot.utils.replace(lang.roulette.starting, {})); //.delay(duration * 1e3).call("delete");

          bot.chat(bot.utils.replace(lang.roulette.info, {
            duration,
