    ...
    async execute(rawData, { args }, lang) {
      ...
      switch (param) {
        case "check": {
          ...
          const cooldown = await bot.redis.getCommandOnCoolDown("main", "russianroulette@start", "perUse");
        ...
        case "reset": {
          const user = await bot.getUsers(rawData.uid);

          //if (!isObject(user) || await bot.utils.getRole(user) <= ROLE.MANAGER) return false;

          await bot.redis.removeCommandFromCoolDown("main", "russianroulette@start", "perUse");
        ...
        case "start": {
          ...
          //if (isWeekend) {
          if (isWeekend && !isDecember) {
            bot.chat(bot.utils.replace(lang.russianroulette.startingWeekend, {})); //.delay(duration * 1e3).call("delete");
          }
          ...
          bot.chat(bot.utils.replace(lang.russianroulette.starting, {})); //.delay(duration * 1e3).call("delete");

          bot.chat(bot.utils.replace(lang.russianroulette.info, {
            duration,
